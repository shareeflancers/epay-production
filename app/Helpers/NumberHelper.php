...
{
    /**
     * Convert a number to its word representation (English).
     * Pure PHP implementation to avoid dependency on the 'intl' extension.
     */
    public static function amountInWords($amount): string
    {
        $amount = (float) $amount;
        $whole = floor($amount);
        $fraction = round(($amount - $whole) * 100);

        $result = ucfirst(self::convertNumberToWords($whole)) . " Rupees";

        if ($fraction > 0) {
            $result .= " and " . self::convertNumberToWords($fraction) . " Paisa";
        }
